...
A browser extension designed to modify the Toddle web interface for better usability. This is a personal project provided as-is.

## Features

* **Subject Aliases:** Allows searching for courses using full names (e.g., "Chemistry") instead of just course codes.
...
4. **Load the extension:** Click **Load unpacked** and select the folder you just extracted (the one containing `manifest.json`, it should be the `noddle` folder. **NOT the `noddle-main` folder**).
5. **Pin it:** Click the puzzle piece icon in your browser toolbar and pin the extension for easy access to settings.

## Updating

To update to a newer version:
1. Go to `chrome://extensions` and remove the old extention
2. Install the new version by following the installation guide again.
